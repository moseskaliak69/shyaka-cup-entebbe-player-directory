...
(() => {
  ...
  const publicPlayers=document.createElement('section');publicPlayers.className='view';publicPlayers.id='view-publicplayers';publicPlayers.innerHTML=`<div class="public-content"><h2 class="public-page-title">Players</h2><div class="public-empty player-coming-soon">${icon('people')}<h3>Player photos coming soon</h3><p>The public photo directory is being prepared.</p><button class="public-outline" data-public-view="teams">Browse villages ${icon('arrow-right')}</button></div></div>`;$('view-players').after(publicPlayers);
  $('mobileBottomNav').innerHTML = [['home','house-fill','Home'],['fixtures','calendar4','Fixtures'],['livescores','broadcast','Live'],['gallery','play-circle','Highlights'],['publicplayers','people','Players']].map(([view,img,label])=>`<button data-public-view="${view}">${icon(img)}<span>${label}</span></button>`).join('');
